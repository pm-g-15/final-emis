  ...
  async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/STAFFs/fees/${staffname}/${staffid}`,
        {
          salaryForTheYear,
          salaryForTheMonth,
          salaryAmount,
...
export const staffregister =
  ...
  async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/staffs/register`,
        {
          staff_name,
...
export const deleteStaff = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `${process.env.REACT_APP_API_URL}/api/staffs/delete/${id}`
    );
    dispatch({
...
export const listStaffs = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API_URL}/api/staffs`
    );
    dispatch({
